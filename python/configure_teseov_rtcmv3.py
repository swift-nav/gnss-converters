...
# construct SETMTI packet with payload fields below

# data field            DF     BITS
# Message Number        DF002   12
# Sub Type ID           DF02P    8
# Target Message Number DF002   12
# Target Sub Type ID    DF02P    8
# MTI                   DF057   16
...
def construct_set_mti_payload(target_sub_type_id, 
                              message_transmission_interval_deciseconds):
    buff = bytearray(7)
    # first 12 bits are message number
    buff[0]  = TESEOV_CUSTOM_MESSAGE_NUMBER >> 4
    buff[1]  = (TESEOV_CUSTOM_MESSAGE_NUMBER & 0x00f) << 4
    buff[1] |= MTI_SUBTYPE_ID >> 4
    buff[2]  = (MTI_SUBTYPE_ID & 0xf) << 4 
    buff[2] |= (TESEOV_CUSTOM_MESSAGE_NUMBER  >> 8) & 0xf 
    buff[3]  = (TESEOV_CUSTOM_MESSAGE_NUMBER  & 0xff)
    buff[4]  = target_sub_type_id
    buff[5]  = (message_transmission_interval_deciseconds >> 8) & 0xff
    buff[6]  = (message_transmission_interval_deciseconds & 0xff)
    #print("paylod bytes are:")
    #for each in buff:
      #print("{0:X}".format(each))
    #print("framing")
    #print(binascii.hexlify(frame_rtcmv3(buff)))
    print(frame_rtcmv3(buff))

construct_set_mti_payload(25, 100)
