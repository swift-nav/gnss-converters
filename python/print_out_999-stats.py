...
def print_summary(read):
    ...
    for msg in source:
        ...
        if msg_type == 999:
            sub_type = ((msg[1] & 0x0f) << 4) | ((msg[2] & 0xf0) >> 4)
            msg_dict[sub_type] = msg_dict.get(sub_type, 0) + len(msg)
    for key, val in msg_dict.items():
        print("msg type {} ({}) had {} bytes [{:.2f}%]".format(key, name_dict.get(key, "unknown"), val, val/total_bytes * 100))
